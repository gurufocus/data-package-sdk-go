/*
Gurufocus Data Package API

API for accessing Gurufocus data packages, please go to [https://www.gurufocus.com/user/me?tab=account&subtab=api-token](https://www.gurufocus.com/user/me?tab=account&subtab=api-token) to view or generate authorization keys.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the FundamentalsNREITDIRECT type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FundamentalsNREITDIRECT{}

// FundamentalsNREITDIRECT struct for FundamentalsNREITDIRECT
type FundamentalsNREITDIRECT struct {
	BalanceSheet *FundamentalsNREITDIRECTBalanceSheet `json:"balance_sheet,omitempty"`
	BasicInformation *FundamentalsNREITDIRECTBasicInformation `json:"basic_information,omitempty"`
	CashflowStatement *FundamentalsNREITDIRECTCashflowStatement `json:"cashflow_statement,omitempty"`
	IncomeStatement *FundamentalsNREITDIRECTIncomeStatement `json:"income_statement,omitempty"`
}

// NewFundamentalsNREITDIRECT instantiates a new FundamentalsNREITDIRECT object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFundamentalsNREITDIRECT() *FundamentalsNREITDIRECT {
	this := FundamentalsNREITDIRECT{}
	return &this
}

// NewFundamentalsNREITDIRECTWithDefaults instantiates a new FundamentalsNREITDIRECT object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFundamentalsNREITDIRECTWithDefaults() *FundamentalsNREITDIRECT {
	this := FundamentalsNREITDIRECT{}
	return &this
}

// GetBalanceSheet returns the BalanceSheet field value if set, zero value otherwise.
func (o *FundamentalsNREITDIRECT) GetBalanceSheet() FundamentalsNREITDIRECTBalanceSheet {
	if o == nil || IsNil(o.BalanceSheet) {
		var ret FundamentalsNREITDIRECTBalanceSheet
		return ret
	}
	return *o.BalanceSheet
}

// GetBalanceSheetOk returns a tuple with the BalanceSheet field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FundamentalsNREITDIRECT) GetBalanceSheetOk() (*FundamentalsNREITDIRECTBalanceSheet, bool) {
	if o == nil || IsNil(o.BalanceSheet) {
		return nil, false
	}
	return o.BalanceSheet, true
}

// HasBalanceSheet returns a boolean if a field has been set.
func (o *FundamentalsNREITDIRECT) HasBalanceSheet() bool {
	if o != nil && !IsNil(o.BalanceSheet) {
		return true
	}

	return false
}

// SetBalanceSheet gets a reference to the given FundamentalsNREITDIRECTBalanceSheet and assigns it to the BalanceSheet field.
func (o *FundamentalsNREITDIRECT) SetBalanceSheet(v FundamentalsNREITDIRECTBalanceSheet) {
	o.BalanceSheet = &v
}

// GetBasicInformation returns the BasicInformation field value if set, zero value otherwise.
func (o *FundamentalsNREITDIRECT) GetBasicInformation() FundamentalsNREITDIRECTBasicInformation {
	if o == nil || IsNil(o.BasicInformation) {
		var ret FundamentalsNREITDIRECTBasicInformation
		return ret
	}
	return *o.BasicInformation
}

// GetBasicInformationOk returns a tuple with the BasicInformation field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FundamentalsNREITDIRECT) GetBasicInformationOk() (*FundamentalsNREITDIRECTBasicInformation, bool) {
	if o == nil || IsNil(o.BasicInformation) {
		return nil, false
	}
	return o.BasicInformation, true
}

// HasBasicInformation returns a boolean if a field has been set.
func (o *FundamentalsNREITDIRECT) HasBasicInformation() bool {
	if o != nil && !IsNil(o.BasicInformation) {
		return true
	}

	return false
}

// SetBasicInformation gets a reference to the given FundamentalsNREITDIRECTBasicInformation and assigns it to the BasicInformation field.
func (o *FundamentalsNREITDIRECT) SetBasicInformation(v FundamentalsNREITDIRECTBasicInformation) {
	o.BasicInformation = &v
}

// GetCashflowStatement returns the CashflowStatement field value if set, zero value otherwise.
func (o *FundamentalsNREITDIRECT) GetCashflowStatement() FundamentalsNREITDIRECTCashflowStatement {
	if o == nil || IsNil(o.CashflowStatement) {
		var ret FundamentalsNREITDIRECTCashflowStatement
		return ret
	}
	return *o.CashflowStatement
}

// GetCashflowStatementOk returns a tuple with the CashflowStatement field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FundamentalsNREITDIRECT) GetCashflowStatementOk() (*FundamentalsNREITDIRECTCashflowStatement, bool) {
	if o == nil || IsNil(o.CashflowStatement) {
		return nil, false
	}
	return o.CashflowStatement, true
}

// HasCashflowStatement returns a boolean if a field has been set.
func (o *FundamentalsNREITDIRECT) HasCashflowStatement() bool {
	if o != nil && !IsNil(o.CashflowStatement) {
		return true
	}

	return false
}

// SetCashflowStatement gets a reference to the given FundamentalsNREITDIRECTCashflowStatement and assigns it to the CashflowStatement field.
func (o *FundamentalsNREITDIRECT) SetCashflowStatement(v FundamentalsNREITDIRECTCashflowStatement) {
	o.CashflowStatement = &v
}

// GetIncomeStatement returns the IncomeStatement field value if set, zero value otherwise.
func (o *FundamentalsNREITDIRECT) GetIncomeStatement() FundamentalsNREITDIRECTIncomeStatement {
	if o == nil || IsNil(o.IncomeStatement) {
		var ret FundamentalsNREITDIRECTIncomeStatement
		return ret
	}
	return *o.IncomeStatement
}

// GetIncomeStatementOk returns a tuple with the IncomeStatement field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FundamentalsNREITDIRECT) GetIncomeStatementOk() (*FundamentalsNREITDIRECTIncomeStatement, bool) {
	if o == nil || IsNil(o.IncomeStatement) {
		return nil, false
	}
	return o.IncomeStatement, true
}

// HasIncomeStatement returns a boolean if a field has been set.
func (o *FundamentalsNREITDIRECT) HasIncomeStatement() bool {
	if o != nil && !IsNil(o.IncomeStatement) {
		return true
	}

	return false
}

// SetIncomeStatement gets a reference to the given FundamentalsNREITDIRECTIncomeStatement and assigns it to the IncomeStatement field.
func (o *FundamentalsNREITDIRECT) SetIncomeStatement(v FundamentalsNREITDIRECTIncomeStatement) {
	o.IncomeStatement = &v
}

func (o FundamentalsNREITDIRECT) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FundamentalsNREITDIRECT) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BalanceSheet) {
		toSerialize["balance_sheet"] = o.BalanceSheet
	}
	if !IsNil(o.BasicInformation) {
		toSerialize["basic_information"] = o.BasicInformation
	}
	if !IsNil(o.CashflowStatement) {
		toSerialize["cashflow_statement"] = o.CashflowStatement
	}
	if !IsNil(o.IncomeStatement) {
		toSerialize["income_statement"] = o.IncomeStatement
	}
	return toSerialize, nil
}

type NullableFundamentalsNREITDIRECT struct {
	value *FundamentalsNREITDIRECT
	isSet bool
}

func (v NullableFundamentalsNREITDIRECT) Get() *FundamentalsNREITDIRECT {
	return v.value
}

func (v *NullableFundamentalsNREITDIRECT) Set(val *FundamentalsNREITDIRECT) {
	v.value = val
	v.isSet = true
}

func (v NullableFundamentalsNREITDIRECT) IsSet() bool {
	return v.isSet
}

func (v *NullableFundamentalsNREITDIRECT) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFundamentalsNREITDIRECT(val *FundamentalsNREITDIRECT) *NullableFundamentalsNREITDIRECT {
	return &NullableFundamentalsNREITDIRECT{value: val, isSet: true}
}

func (v NullableFundamentalsNREITDIRECT) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFundamentalsNREITDIRECT) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


